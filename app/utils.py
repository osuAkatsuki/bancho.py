...
def ensure_dependencies_and_requirements() -> int:
    ...
    if not OSU_TOOLS_PATH.exists():
        log("No osu-tools submodule found, attempting to clone.", Ansi.LMAGENTA)
        p = subprocess.Popen(
    ...
    if not OSU_TOOLS_EXEC_PATH.exists():
        log("No osu-tools binary found, attempting to build.", Ansi.LMAGENTA)
        p = subprocess.Popen(
            args=["dotnet", "build"],
            cwd="osu-tools",
            stdout=subprocess.DEVNULL,
        ...
        if exit_code := p.wait():
            ...
            print(stderr.decode())
            log("Failed to build osu-tools automatically.", Ansi.LRED)
            return exit_code
        if not OSU_TOOLS_EXEC_PATH.exists():
            log("Failed to build osu-tools, aborting.", Ansi.LRED)
            return 1
