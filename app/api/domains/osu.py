...
async def getScores(
    ...
    if not bmap:
        ...
        bmap_set = await BeatmapSet.from_bsid(row["set_id"])
        assert bmap_set is not None

        new_bmap = Beatmap(**row, map_set=bmap_set)
        ...
        if requested_md5 != map_md5:
            ...
            # the map is a match with the one generated by the client
            # let them continue through the regular handler behaviour
            bmap = new_bmap
