...
from mount.app.objects.menu import menu_keygen
...
if TYPE_CHECKING:
    ...
    from mount.app.objects.clan import Clan, ClanPrivileges
...
class Player:
    ...
    def __init__(
        ...
        self.geoloc: services.Geolocation = extras.get(
            "geoloc",
            {
                "latitude": 0.0,
                "longitude": 0.0,
    ...
    async def restrict(self, admin: "Player", reason: str) -> None:
        ...
        if webhook_url := glob.config.webhooks["audit-log"]:
            ...
            await webhook.post(services.http)
    ...
    async def silence(self, admin: "Player", duration: int, reason: str) -> None:
        ...
        sessions.players.enqueue(packets.user_silenced(self.id))
    ...
    def join_match(self, m: Match, passwd: str) -> bool:
        ...
        if self is not m.host:
            ...
            if passwd != m.passwd and self not in sessions.players.staff:
        ...
        if (lobby := sessions.channels["#lobby"]) in self.channels:
    ...
    def leave_match(self) -> None:
        ...
        if all(map(Slot.empty, self.match.slots)):
            ...
            sessions.matches.remove(self.match)

            if lobby := sessions.channels["#lobby"]:
    ...
    def add_spectator(self, p: "Player") -> None:
        ...
        if not (spec_chan := sessions.channels[chan_name]):
            ...
            sessions.channels.append(spec_chan)
    ...
    def remove_spectator(self, p: "Player") -> None:
        ...
        c = sessions.channels[f"#spec_{self.id}"]
        p.leave_channel(c)
    ...
    async def add_friend(self, p: "Player") -> None:
        ...
        await services.database.execute(
            db_models.relationships.insert().values(user1=self.id, user2=p.id),
        )
    ...
    async def remove_friend(self, p: "Player") -> None:
        ...
        await services.database.execute(
            db_models.relationships.delete().where(
                sqlalchemy.and_(
                    db_models.relationships.c.user1 == self.id,
                    db_models.relationships.c.user2 == p.id,
                ),
            ),
        )
    ...
    async def add_block(self, p: "Player") -> None:
        ...
        insert_data = insert(db_models.relationships).values(
            user1=self.id,
            user2=p.id,
            type="block",
        )
        ...
        await services.database.execute(duplicate_format)
    ...
    async def unlock_achievement(self, a: "Achievement") -> None:
        ...
        await services.database.execute(
            db_models.user_achievements.insert().values(userid=self.id, achid=a.id),
        )
    ...
    async def relationships_from_sql(self, db_conn: databases.core.Connection) -> None:
        ...
        async for row in db_conn.iterate(
            select(
                [
                    db_models.relationships.c.user2,
                    db_models.relationships.c.type,
                ],
            ).where(db_models.relationships.c.user1 == self.id),
        ):
            if row["type"] == "friend":
    ...
    def send_menu_clear(self) -> None:
        ...
        self.enqueue(packets.user_silenced(sessions.bot.id))
