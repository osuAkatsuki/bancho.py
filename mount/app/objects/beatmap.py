            ...
            async for row in db_conn.iterate(
                select(
                    [
                        ...
                        db_models.maps.c.filename,
                        db_models.maps.c.last_update,
                        db_models.maps.c.total_length,
                        db_models.maps.c.max_combo,
                        db_models.maps.c.status,
                        db_models.maps.c.frozen,
                        db_models.maps.c.plays,
                        db_models.maps.c.passes,
                        db_models.maps.c.mode,
                        db_models.maps.c.bpm,
                        db_models.maps.c.cs,
                        db_models.maps.c.od,
                        db_models.maps.c.ar,
                        db_models.maps.c.hp,
        ...
        if api_data := await osuapiv1_getbeatmaps(s=bsid):
            ...
            async with services.database.connection() as db_conn:
                res = await db_conn.fetch_all(
                    select([db_models.maps.c.id, db_models.maps.c.status]).where(
                        sqlalchemy.and_(
                            db_models.maps.c.set_id == bsid,
                            db_models.maps.c.frozen == 1,
                        ),
                    ),
                )
