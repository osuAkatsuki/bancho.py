...
from mount.app.utils import make_safe_name
...
class Channels(list[Channel]):
    ...
    async def prepare(self, db_conn: databases.core.Connection) -> None:
        ...
        channel_res = await db_conn.fetch_all(db_models.channels.select())
        self.extend(
            [
                Channel(
                    name=row["name"],
            ...
            row = await db_conn.fetch_one(
                select(
                    [
                        ...
                        db_models.users.c.pw_bcrypt,
                        db_models.users.c.silence_end,
                        db_models.users.c.clan_id,
...
class MapPools(list[MapPool]):
    ...
    async def prepare(self, db_conn: databases.core.Connection) -> None:
        ...
        pool_res = await db_conn.fetch_all(db_models.tourney_pools.select())
        self.extend(
            [
                MapPool(
                    id=row["id"],
                    ...
                    created_by=await sessions.players.from_cache_or_sql(
                        id=row["created_by"],
                    ),
                )
                for row in pool_res
