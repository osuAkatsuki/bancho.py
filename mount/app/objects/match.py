        ...
        for row in await db_conn.fetch_all(
            select(
                [
                    ...
                    db_models.tourney_pool_maps.c.mods,
                    db_models.tourney_pool_maps.c.slot,
                ],
            ).where(db_models.tourney_pool_maps.c.pool_id == self.id),
        ):
            map_id = row["map_id"]
            ...
            if not bmap:
                ...
                await db_conn.execute(
                    db_models.tourney_pool_maps.delete().where(
                        db_models.tourney_pool_maps.c.map_id == map_id,
                    ),
                )
...
class Match:
    ...
    def enqueue(
        ...
        if lobby and (lchan := sessions.channels["#lobby"]) and lchan.players:
            lchan.enqueue(data)
