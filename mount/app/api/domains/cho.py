...
from mount.app.constants import commands
...
from mount.app.constants.privileges import ClientPrivileges
...
from mount.app.objects.match import Slot
from mount.app.objects.match import SlotStatus
from mount.app.objects.menu import Menu
from mount.app.objects.menu import MenuCommands
from mount.app.objects.menu import MenuFunction
from mount.app.objects.player import Action
from mount.app.objects.player import Player
from mount.app.objects.player import PresenceFilter
    ...
    return HTMLResponse(
        ...
        + "<br>".join(
            (
                ...
                f"Players online: {len(sessions.players) - 1}",
...
class SendMessage(BasePacket):
    ...
    async def handle(self, p: Player) -> None:
        ...
        if recipient in ("#highlight", "#userlog"):
            ...
            t_chan = sessions.channels[f"#spec_{spec_id}"]
            ...
            t_chan = sessions.channels[recipient]
...
WELCOME_NOTIFICATION = packets.notification(
    f"Welcome back to {settings.DOMAIN}!\n",  # f"Running gulag v{glob.version}.",
)
...
async def login(
    ...
    Some notes:
      this must be called with sessions.players._lock held.
    ...
    if not using_tourney_client:
        ...
        if p := sessions.players.get(name=username):
    ...
    user_info = await db_conn.fetch_one(
        db_models.users.select().where(
            db_models.users.c.safe_name == utils.make_safe_name(username),
        ),
    )
    ...
    if not user_info:
        ...
        return "no", (
            packets.notification(f"{settings.DOMAIN}: Unknown username")
            + packets.user_id(-1)
        )
    ...
    if pw_bcrypt in bcrypt_cache:  # ~0.01 ms
        if pw_md5 != bcrypt_cache[pw_bcrypt]:
            return "no", (
                packets.notification(f"{settings.DOMAIN}: Incorrect password")
                + packets.user_id(-1)
            )
    else:  # ~200ms
    ...
    if is_wine:
        ...
        hw_args.extend(
            [
                db_models.client_hashes.c.adapters == adapters_md5,
                db_models.client_hashes.c.uninstall_id == uninstall_md5,
                db_models.client_hashes.c.disk_serial == disk_sig_md5,
            ],
        )
    ...
    if user_info["clan_id"] != 0:
        clan = sessions.clans.get(id=user_info.pop("clan_id"))
    ...
    if not ip.is_private:
        if services.geoloc_db is not None:
            ...
            user_info["geoloc"] = services.fetch_geoloc_db(ip)
        else:
            # bad, we must do an external db lookup using
            # a public api. (depends, `ping ip-api.com`)
            user_info["geoloc"] = await services.fetch_geoloc_web(ip)
        ...
        if db_country == "xx":
            ...
            await db_conn.execute(
                ...
                .where(db_models.users.c.id == user_info["id"]),
            )
    ...
    if not p.restricted:
        ...
        if not p.priv & Privileges.VERIFIED:
            ...
            data += packets.send_message(
                sender=sessions.bot.name,
                msg=WELCOME_MSG,
                recipient=p.name,
                sender_id=sessions.bot.id,
            )

    else:
        # player is restricted, one way data
        for o in sessions.players.unrestricted:
            # enqueue them to us.
            data += packets.user_presence(o)
            data += packets.user_stats(o)
...
class StartSpectating(BasePacket):
    ...
    async def handle(self, p: Player) -> None:
        if not (new_host := sessions.players.get(id=self.target_id)):
...
class SendPrivateMessage(BasePacket):
    ...
    async def handle(self, p: Player) -> None:
        ...
        if not (t := await sessions.players.from_cache_or_sql(name=t_name)):
            if settings.DEBUG:
...
class MatchCreate(BasePacket):
    ...
    async def handle(self, p: Player) -> None:
        ...
        if not sessions.matches.append(self.match):
...
class MatchJoin(BasePacket):
    ...
    async def handle(self, p: Player) -> None:
        ...
        if not (m := sessions.matches[self.match_id]):
...
class FriendAdd(BasePacket):
    ...
    async def handle(self, p: Player) -> None:
        if not (t := sessions.players.get(id=self.user_id)):
...
class StatsRequest(BasePacket):
    ...
    async def handle(self, p: Player) -> None:
        ...
        for online in filter(is_online, self.user_ids):
            if t := sessions.players.get(id=online):
...
class UserPresenceRequest(BasePacket):
    ...
    async def handle(self, p: Player) -> None:
        for pid in self.user_ids:
            if t := sessions.players.get(id=pid):
...
class UserPresenceRequestAll(BasePacket):
    ...
    async def handle(self, p: Player) -> None:
        ...
        p.enqueue(
            b"".join(map(packets.user_presence, sessions.players.unrestricted)),
        )
