...
async def api_get_player_count(
    ...
    total_users = await db_conn.fetch_val(
        select(func.count()).select_from(db_models.users),
        column=0,
    )
    ...
    return ORJSONResponse(
        {
            "status": "success",
            "counts": {
                "online": len(sessions.players.unrestricted) - 1,  # -1 for bot
                "total": total_users,
            },
        },
    )
...
async def api_get_player_info(
        ...
        user_info = await db_conn.fetch_one(
            select(
                [
                    ...
                    db_models.users.c.safe_name,
                    db_models.users.c.priv,
                    db_models.users.c.country,
                    db_models.users.c.silence_end,
                ],
            ).where(db_models.users.c.safe_name == username.lower()),
        )
    else:  # if user_id
        user_info = await db_conn.fetch_one(
            select(
            ...
            ).where(db_models.users.c.userid == user_id),
        )
    ...
    if scope in ("stats", "all"):
        ...
        rows = await db_conn.fetch_all(
            select(
                [
                    db_models.stats.c.tscore,
                    db_models.stats.c.rscore,
                    db_models.stats.c.pp,
                    db_models.stats.c.plays,
                    db_models.stats.c.playtime,
                    db_models.stats.c.acc,
                    db_models.stats.c.max_combo,
                    db_models.stats.c.xh_count,
                    db_models.stats.c.x_count,
                    db_models.stats.c.sh_count,
                    db_models.stats.c.s_count,
                    db_models.stats.c.a_count,
                ],
            ).where(db_models.stats.c.userid == resolved_user_id),
        )
        ...
        for idx, mode_stats in enumerate([dict(row) for row in rows]):
            ...
            country_rank = await services.redis.zrevrank(
...
async def api_get_player_status(
    ...
    if username:
        player = sessions.players.get(name=username)
    elif user_id:
        player = sessions.players.get(id=user_id)
    else:
        return ORJSONResponse(
            {"status": "Must provide either id OR name!"},
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    if not player:
        ...
        if username:
            row = await db_conn.fetch_one(
                db_models.users.select(db_models.users.c.latest_activity).where(
                    db_models.users.c.safe_name == username.lower(),
                ),
            )
        else:  # if user_id
            row = await db_conn.fetch_one(
                db_models.users.select(db_models.users.c.latest_activity).where(
                    db_models.users.c.id == user_id,
                ),
            )
...
async def api_get_player_scores(
    ...
    if username:
        player = await sessions.players.from_cache_or_sql(name=username)
    elif user_id:
        player = await sessions.players.from_cache_or_sql(id=user_id)
    else:
        return ORJSONResponse(
            {"status": "Must provide either id OR name!"},
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    alchemy_table = getattr(db_models, mode.scores_table)
    ...
    if scope == "best":
        ...
        params.append(
            alchemy_table.c.status == 2 and db_models.maps.c.status in allowed_statuses,
        )
    ...
    return ORJSONResponse({"status": "success", "scores": rows, "player": player_info})
...
async def api_get_player_most_played(
    ...
    if id is not None:
        p = await sessions.players.from_cache_or_sql(id=id)
    elif username is not None:
        p = await sessions.players.from_cache_or_sql(name=username)
    else:
        return ORJSONResponse(
            {"status": "Must provide either id or name."},
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def api_get_map_scores(
    ...
    user_join = join(
        alchemy_table,
        db_models.users,
        alchemy_table.c.userid == db_models.users.c.id,
    )
    user_clan_join = join(
        user_join,
        db_models.clans,
        db_models.users.c.clan_id == db_models.clans.c.id,
    )
    res = await db_conn.fetch_all(
        select(
            [
                ...
                alchemy_table.c.perfect,
                db_models.users.c.name.label("player_name"),
                db_models.clans.c.id.label("clan_id"),
                db_models.clans.c.name.label("clan_name"),
                db_models.clans.c.tag.label("clan_tag"),
            ],
        )
...
async def api_get_replay(
    ...
    res = await db_conn.fetch_one(
        select(
            [
                db_models.users.c.name.label("username"),
                db_models.maps.c.md5.label("map_md5"),
                db_models.maps.c.artist,
                db_models.maps.c.title,
                db_models.maps.c.version,
                alchemy_table.c.mode,
...
async def api_get_match(
    ...
    if not (match := sessions.matches[match_id]):
        return ORJSONResponse(
...
async def api_get_global_leaderboard(
    ...
    alchemy_sort = getattr(db_models.stats.c, sort)
    users_join = join(
        db_models.stats,
        db_models.users,
        db_models.stats.c.id == db_models.users.c.id,
    )
    users_clans_join = join(
        users_join,
        db_models.clans,
        db_models.clans.c.id == db_models.users.c.clan_id,
    )
    res = await db_conn.fetch_all(
        select(
            [
                db_models.users.c.id.label("player_id"),
                db_models.users.c.name,
                db_models.users.c.country,
                db_models.stats.c.tscore,
                db_models.stats.c.rscore,
        ...
        .where(
            sqlalchemy.and_(
                db_models.stats.c.mode == mode.value,
...
#         p = await sessions.players.from_cache_or_sql(id=player_id)
