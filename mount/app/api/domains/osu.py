...
from mount.app import utils
from mount.app.constants import regexes
from mount.app.constants.clientflags import ClientFlags
...
from mount.app.objects.beatmap import Beatmap
from mount.app.objects.beatmap import ensure_local_osu_file
from mount.app.objects.beatmap import RankedStatus
from mount.app.objects.player import Privileges
from mount.app.objects.score import Grade
from mount.app.objects.score import Score
from mount.app.objects.score import SubmissionStatus
...
async def osuError(
    ...
    if username and pw_md5:
        if not (
            player := await sessions.players.from_login(
                name=unquote(username),
                pw_md5=pw_md5,
            )
        ):
            # player login incorrect
            await services.log_strange_occurrence("osu-error auth failed")
            player = None
    else:
        player = None
...
async def osuScreenshot(
    ...
    if endpoint_version != 1:
        await services.log_strange_occurrence(
    ...
    for idx, map_filename in enumerate(form_data.Filenames):
        ...
        row = await db_conn.fetch_one(
            select(
                [
                    db_models.maps.c.id,
                    db_models.maps.c.set_id,
                    db_models.maps.c.status,
                    db_models.maps.c.md5,
                ],
            ).where(db_models.maps.c.filename == map_filename),
        )
        ...
        score_res = await db_conn.fetch_all(
            select([db_models.scores_rx.c.grade, db_models.scores_rx.c.mode]).where(
                sqlalchemy.and_(
                    db_models.scores_rx.c.map_md5 == row["md5"],
                    db_models.scores_rx.c.userid == player.id,
                    db_models.scores_rx.c.status == 2,
                ),
            ),
        )
...
async def osuGetFavourites(
    ...
    favourites = await db_conn.fetch_all(
        db_models.favourites.select(db_models.favourites.c.setid).where(
            db_models.favourites.c.userid == player.id,
        ),
    )
...
async def osuAddFavourite(
    ...
    res = await db_conn.fetch_one(
        db_models.favourites.select().where(
            sqlalchemy.and_(
                db_models.favourites.c.userid == player.id,
                db_models.favourites.c.setid == map_set_id,
            ),
        ),
    )
    ...
    await db_conn.execute(
        insert(db_models.favourites).values(id=player.id, setid=map_set_id),
    )
...
async def lastFM(
    ...
    if flags & ClientFlags.REGISTRY_EDITS:
        ...
        if random.randrange(32) == 0:
            ...
            await player.restrict(admin=sessions.bot, reason="hq!osu relife 1/32")
            return b"-3"
...
async def osuSearchHandler(
    ...
    async with services.http.get(search_url, params=params) as resp:
        if not resp:
            stacktrace = utils.get_appropriate_stacktrace()
            await services.log_strange_occurrence(stacktrace)
...
async def osuSearchSetHandler(
    ...
    if map_set_id is not None:
        ...
        k, v = (db_models.maps.c.set_id, map_set_id)
    elif map_id is not None:
        k, v = (db_models.maps.c.id, map_id)
    else:
        return  # invalid args
    ...
    bmapset = await db_conn.fetch_one(
        select(
            [
                db_models.maps.c.set_id,
                db_models.maps.c.artist,
                db_models.maps.c.title,
                db_models.maps.c.status,
                db_models.maps.c.creator,
                db_models.maps.c.last_update,
            ],
        )
        .where(k == v)
        .distinct(),
...
async def osuSubmitModularSelector(
    ...
    if not (player := await sessions.players.from_login(username, pw_md5)):
    ...
    if score.mode != score.player.status.mode:
        ...
        if not score.player.restricted:
            sessions.players.enqueue(packets.user_stats(score.player))

    scores_table = score.mode.scores_table
    mode_vn = score.mode.as_vanilla

    # Check for score duplicates
    alchemy_table = getattr(db_models, scores_table)
    ...
    if score.status == SubmissionStatus.BEST:
        ...
        if score.bmap.has_leaderboard:
            ...
            if score.rank == 1 and not score.player.restricted:
                ...
                announce_chan = sessions.channels["#announce"]
                ...
                join_table = join(
                    alchemy_table,
                    db_models.users,
                    db_models.users.c.id == alchemy_table.c.userid,
                )
                prev_n1 = await db_conn.fetch_one(
                    select([db_models.users.c.id, db_models.users.c.name])
    ...
    if score.passed and score.bmap.has_leaderboard:
        ...
        if score.bmap.awards_ranked_pp and score.status == SubmissionStatus.BEST:
            ...
            plays_table = join(
                db_models.maps,
                alchemy_table,
                alchemy_table.c.map_md5 == db_models.maps.c.md5,
            )
            ...
            rows = await db_conn.fetch_all(
                ...
                .where(
                    sqlalchemy.and_(
                        ...
                        alchemy_table.c.mode == mode_vn,
                        alchemy_table.c.status == 2,
                        db_models.maps.c.status in (2, 3),
                    ),
                )
    ...
    await db_conn.execute(
        update(db_models.stats, values=stats_query_values).where(
            sqlalchemy.and_(
                db_models.stats.c.userid == score.player.id,
                db_models.stats.c.mode == score.mode.value,
            ),
        ),
    )
    sessions.players.enqueue(packets.user_stats(score.player))
...
async def osuRate(
    ...
    if rating is None:
        ...
        res = await db_conn.fetch_one(
            db_models.ratings.select().where(
                sqlalchemy.and_(
                    db_models.ratings.c.map_md5 == map_md5,
                    db_models.ratings.c.userid == player.id,
                ),
            ),
        )
        ...
        await db_conn.execute(
            db_models.ratings.insert().values(
                userid=player.id,
                map_md5=map_md5,
                rating=rating,
            ),
    ...
    ratings_res = await db_conn.fetch_all(
        db_models.ratings.select(db_models.ratings.c.ratings).where(
            db_models.ratings.c.map_md5 == map_md5,
        ),
    )
...
async def getScores(
    ...
    if mode != player.status.mode:
        ...
        if not player.restricted:
            sessions.players.enqueue(packets.user_stats(player))
        ...
        if has_set_id:
            ...
            map_res = await db_conn.fetch_one(
                db_models.maps.select().where(
                    db_models.maps.c.filename == map_filename,
                ),
            )
    ...
    alchemy_score_table = getattr(db_models, scores_table)
    users_join = join(
        alchemy_score_table,
        db_models.users,
        alchemy_score_table.c.userid == db_models.users.c.id,
    )
    alchemy_table = join(
        users_join,
        db_models.clans,
        users_join.c.clan_id == db_models.clans.c.id,
    )
    ...
    params = [
        ...
        db_models.users.c.priv & 1 or alchemy_table.c.userid == player.id,
        alchemy_table.c.mode == mode_vn,
        alchemy_table.c.status == 2,
    ]
    ...
    if leaderboard_type == LeaderboardType.Mods:
        ...
        params.append(
            db_models.users.c.country == player.geoloc["country"]["acronym"],
        )
    ...
    rating = await db_conn.fetch_val(
        db_models.ratings.select(func.avg(db_models.ratings.c.rating)).where(
            db_models.ratings.c.map_md5 == map_md5,
        ),
        column=0,
    )
...
async def osuComment(
    ...
    if action == "get":
        ...
        comment_user_join = join(
            db_models.comments,
            db_models.users,
            db_models.users.c.id == db_models.comments.c.userid,
        )
        comments = await db_conn.fetchall(
            select(
                [
                    db_models.comments.c.time,
                    db_models.comments.c.target_type,
                    db_models.comments.c.colour,
                    db_models.comments.c.comment,
            ...
            .where(
                sqlalchemy.or_(
                    db_models.comments.c.target_type == "replay"
                    and db_models.comments.c.target_id == score_id,
                    db_models.comments.c.target_type == "map"
                    and db_models.comments.c.target_id == beatmap_id,
                    db_models.comments.c.target_type == "song"
                    and db_models.comments.c.target_id == beatmap_set_id,
                ),
            ),
        )
...
async def osuMarkAsRead(
    ...
    if t := await sessions.players.from_cache_or_sql(name=t_name):
        # mark any unread mail from this user as read.
        await db_conn.execute(
            db_models.mail.update()
            .values(read=1)
            .where(
                sqlalchemy.and_(
                    db_models.mail.c.to_id == player.id,
                    db_models.mail.c.from_id == t.id,
...
async def checkUpdates(
    ...
    async with services.http.get(url, params=request.query_params) as resp:
...
async def get_screenshot(
    ...
    return FileResponse(
        screenshot_path,
        media_type=utils.get_media_type(extension),
    )
...
async def get_osz(
    ...
    return RedirectResponse(
        url=f"{settings.MIRROR_URL}/{query_str}",
        status_code=status.HTTP_301_MOVED_PERMANENTLY,
    )
...
async def get_updated_beatmap(
    ...
    row = await db_conn.fetch_one(
        select([db_models.maps.c.id, db_models.maps.c.md5]).where(
            db_models.maps.c.filename == map_filename,
        ),
    )
    ...
    if (
        ...
        async with services.http.get(url) as resp:
            if not resp or resp.status != 200:
...
async def register_account(
    ...
    if "username" not in errors:
        res = await db_conn.fetch_one(
            db_models.users.select().where(
                db_models.users.c.safe_name == safe_name,
            ),
        )
    ...
    if not regexes.EMAIL.match(email):
        ...
        res = await db_conn.fetch_one(
            db_models.users.select().where(db_models.users.c.email == email),
        )
    ...
    if check == 0:
        ...
        async with sessions.players._lock:
