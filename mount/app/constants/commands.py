...
from mount.app.constants.mods import SPEED_CHANGING_MODS
...
from mount.app.objects.match import SlotStatus
...
async def block(ctx: Context) -> Optional[str]:
    ...
    target = await sessions.players.from_cache_or_sql(name=" ".join(ctx.args))

    if not target:
        return "User not found."

    if target is sessions.bot or target is ctx.player:
        return "What?"
...
async def changename(ctx: Context) -> Optional[str]:
    ...
    if await services.database.fetch_one(
        db_models.users.select().where(db_models.users.c.name == name),
    ):
...
async def recent(ctx: Context) -> Optional[str]:
    ...
    if ctx.args:
        if not (target := sessions.players.get(name=" ".join(ctx.args))):
...
async def top(ctx: Context) -> Optional[str]:
    ...
    if args_len == 2:
        ...
        if not (p := await sessions.players.from_cache_or_sql(name=ctx.args[1])):
            return "Player not found."
    else:
    ...
    alchemy_table = getattr(db_models, table)

    scores = await services.database.fetch_all(
        select([alchemy_table, db_models.maps.c.id.label("bmapid")])
        .where(
            sqlalchemy.and_(
...
async def _with(ctx: Context) -> Optional[str]:
    ...
    if ctx.recipient is not sessions.bot:
...
async def request(ctx: Context) -> Optional[str]:
    ...
    await services.database.execute(
        db_models.map_requests.insert().values(
            map_id=bmap.id,
...
async def get_apikey(ctx: Context) -> Optional[str]:
    ...
    if ctx.recipient is not sessions.bot:
        return f"Command only available in DMs with {sessions.bot.name}."
...
async def requests(ctx: Context) -> Optional[str]:
    ...
    res = await services.database.fetch_all(
        select(
            [
                db_models.map_requests.c.map_id,
                db_models.map_requests.c.player_id,
                db_models.map_requests.c.datetime,
            ],
        ).where(db_models.map_requests.c.active == 1),
    )
    ...
    for (map_id, player_id, dt) in res:
        ...
        if not (p := await sessions.players.from_cache_or_sql(id=player_id)):
    ...
    async with services.database.connection() as db_conn:
        if ctx.args[1] == "set":
            ...
            await db_conn.execute(
                ...
                .where(db_models.maps.c.set_id == bmap.set_id),
            )

            # select all map ids for clearing map requests.
            maps = await db_conn.fetch_all(
                db_models.maps.select(db_models.maps.c.id).where(
                    db_models.maps.c.set_id == bmap.set_id,
                ),
            )
        ...
        for map_id in map_ids:
            await db_conn.execute(
                db_models.map_requests.update()
                .values(active=0)
                .where(db_models.map_requests.c.map_id == map_id),
            )
...
async def notes(ctx: Context) -> Optional[str]:
    ...
    if not (t := await sessions.players.from_cache_or_sql(name=ctx.args[0])):
    ...
    res = await services.database.fetch_all(
        select(
            [
                db_models.logs.c.msg,
                db_models.logs.c.time,
            ],
        )
        .where(
            sqlalchemy.and_(
                db_models.logs.c.to == t.id,
                func.unix_timestamp(db_models.logs.c.time)
                >= func.unix_timestamp(func.now()) - days * 86400,
            ),
        )
        .order_by(db_models.logs.c.time.asc()),
    )
...
async def user(ctx: Context) -> Optional[str]:
    ...
    if not ctx.args:
        ...
        p = await sessions.players.from_cache_or_sql(name=" ".join(ctx.args))
...
async def alert(ctx: Context) -> Optional[str]:
    ...
    sessions.players.enqueue(packets.notification(notif_txt))
    return "Alert sent."
...
async def shutdown(ctx: Context) -> Optional[str]:
    ...
    if ctx.args:  # shutdown after a delay
        ...
        if len(ctx.args) > 1:
            ...
            sessions.players.enqueue(packets.notification(alert_msg))
...
async def fakeusers(ctx: Context) -> Optional[str]:
    ...
    if action == "add":
        ...
        sessions.players.extend(new_fakes)
        del new_fakes
    ...
    for o in [x for x in sessions.players if x.id < FAKE_ID_START]:
        o.enqueue(data)
...
async def recalc(ctx: Context) -> Optional[str]:
    ...
    if ctx.args[0] == "map":
        ...
        staff_chan = sessions.channels["#staff"]  # log any errs here
        ...
        async def recalc_all() -> None:
            ...
            async with services.database.connection() as db_conn:
                maps = await db_conn.fetch_all(
                    ...
                    ).where(db_models.maps.c.passes > 0),
                )
            ...
            elapsed = utils.seconds_readable(int(time.time() - st))
...
async def wipemap(ctx: Context) -> Optional[str]:
    ...
    async with services.database.connection() as db_conn:
        for t in ("vn", "rx", "ap"):
            alchemy_table = getattr(db_models, f"scores_{t}")
            await db_conn.execute(
...
if glob.config.advanced:
    ...
    async def py(ctx: Context) -> Optional[str]:
        ...
        # Ex: !py return (await sessions.players.get(name='cmyui')).status.action
        if not ctx.args:
            return "owo"
...
async def pool_create(ctx: Context) -> Optional[str]:
    ...
    if sessions.pools.get_by_name(name):
        return "Pool already exists by that name!"

    # insert pool into db
    await services.database.execute(
        db_models.tourney_pools.insert().values(
            name=name,
            created_at=func.now(),
    ...
    res = await services.database.fetch_one(
        select([db_models.tourney_pools]).where(
            db_models.tourney_pools.c.name == name,
        ),
    )

    res["created_by"] = await sessions.players.from_cache_or_sql(
        id=res["created_by"],
    )

    sessions.pools.append(MapPool(**res))
...
async def pool_delete(ctx: Context) -> Optional[str]:
    ...
    # delete from db
    await services.database.execute(
        db_models.tourney_pools.delete().where(
            db_models.tourney_pools.c.name == name,
        ),
    )
...
async def pool_add(ctx: Context) -> Optional[str]:
    ...
    # insert into db
    await services.database.execute(
        db_models.tourney_pool_maps.insert().values(
            map_id=bmap.id,
            pool_id=pool.id,
            mods=mods,
            slot=slot,
...
async def pool_remove(ctx: Context) -> Optional[str]:
    ...
    # delete from db
    await services.database.execute(
        db_models.tourney_pool_maps.delete().where(
            sqlalchemy.and_(
                db_models.tourney_pool_maps.c.mods == mods,
                db_models.tourney_pool_maps.c.slot == slot,
            ),
        ),
    )
...
async def clan_create(ctx: Context) -> Optional[str]:
    ...
    if sessions.clans.get(name=name):
        return "That name has already been claimed by another clan."

    if sessions.clans.get(tag=tag):
    ...
    if announce_chan := sessions.channels["#announce"]:
...
async def clan_disband(ctx: Context) -> Optional[str]:
    ...
    if ctx.args:
        ...
        if ctx.player not in sessions.players.staff:
            return "Only staff members may disband the clans of others."

        if not (clan := sessions.clans.get(tag=" ".join(ctx.args).upper())):
    ...
    await services.database.execute(
        db_models.clans.delete().where(db_models.clans.c.id == clan.id),
    )
    ...
    for member_id in clan.members:
        if member := sessions.players.get(id=member_id):
            member.clan = None
    ...
    await services.database.execute(
        ...
        .where(db_models.users.c.clan_id == clan.id),
    )
...
async def clan_info(ctx: Context) -> Optional[str]:
    ...
    res = await services.database.fetch_all(
        ...
        .where(db_models.users.c.clan_id == clan.id)
        .order_by(db_models.users.c.clan_priv.desc()),
    )
...
async def clan_list(ctx: Context) -> Optional[str]:
    ...
    if offset >= (total_clans := len(sessions.clans)):
        return "No clans found."

    msg = [f"gulag clans listing ({total_clans} total)."]

    for idx, clan in enumerate(sessions.clans, offset):
